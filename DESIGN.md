...
1. 注册三个类型（`legacy.conversation` / `legacy.sidebar` / `legacy.rightbar`）。两个列类型里，左栏是 `{ grows: false, closable: false }`——它从不消失，也没有谁能把它要回来；右栏**必须可关**，因为"隐藏"就是关掉它那一格，误关则由下一次对账补回来；
