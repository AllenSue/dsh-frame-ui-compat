...
> ⚠ **两处偏离出厂、必须记住的地方**：
> ① 出厂的三列由 `computeColumns` 从**偏好**解出，所以开右栏只会压缩**中央**；这里右栏那一列的宽度同样是核心给的，但**左栏声明了固定宽度**（`grows: false`，见 §6），所以让出位置的始终是中央——与出厂的 `280px minmax(0, 1fr) 0` 一致。曾经不是这样：`resizePane` 按兄弟比例分摊，右栏占位时左栏也窄一点，由 compat 在收栏时补回去（那条棘轮用例仍在）。T26 把这条账从"事后补偿"改成了核心规则。
> ② 右栏的 frame 宽度就是面板宽度（同一个数），但**不是** owner props 里的 `width`：`width` 是"若显示会占多宽"，frame 是"现在实际占了多宽"。两者在显示时相等，在隐藏时后者不存在。
>
> **两列的宽度都是"像素偏好 + 规则"，不是份额（T30）。** frame 树里存的是**占比**，所以一格按启动视口算出来的宽度会随窗口变化：1000px 窗口里的 280px 轨，在 1920px 窗口里变成 538px——桌面版"左右边栏过宽"就是这么来的（浏览器通常一开就是最大化，所以只有桌面版显形）。出厂的规则是两条，都照抄：**宽度是像素**（`SIDEBAR_DEFAULT` 280，拖动夹在 `[264, 420]`；右栏是记住的像素，没有才用首开 45%），以及**窄框自动折叠**（`viewport < SIDEBAR_AUTO_COLLAPSE` = 1024 时左栏画成 56px 图标轨，除非用户在窄框里手动展开过——出厂 `stores.ts` 的 `narrowExpanded`）。宽度与这两个决定由**本层**记在 `localStorage` 的 `dsh.frames.columns` 里，因为出厂也是这么做的（它的 client store 存着 `sidebar` / `rightbar` / `narrowExpanded`）：**这正是 45% 那个首开默认值平时见不到的原因**——用户拖过一次，偏好就留下了。对账每一轮都重新要一次这两个宽度，所以预设里那一格的占比不会覆盖宽度偏好：**宽度是偏好，编队是布局。**
>
> **面板的内侧边（与中央之间那条线）由这一层画。** 渲染端在每个边界上画了一根细线，但**这一条它画不到**：这一列的像素是在 overlay 上画的（不在那一格的 body 里），而 overlay 在渲染端的线**之上**，所以线被面板盖住——表现就是"呼出右栏后看不到会话与右栏之间的边界"。出厂是同一个格局、同一个答案，而且写在它的样式表里：*占据者自己画左边那条边*（`AppFrame.module.css`：`.rightbarCol` 永不裁剪，注释原话 "The occupant draws its own left border"）。所以 `overlay-body.ts` 给那个盒子加一条 `borderLeft`，并显式用 `content-box`——面板必须保持与列宽**逐像素**一致（那是这一列算术的全部意义），边框要占这个盒子从中央拿来的那一像素，而不是面板自己的一像素。隐藏时（盒子 0 宽）不画：没有那一格，就没有哪条边界。
...
1. 注册三个类型（`legacy.conversation` / `legacy.sidebar` / `legacy.rightbar`）。两个列类型里，左栏是 `{ grows: false }`——旁边的格子关掉时它不跟着长；右栏**必须可关**，因为"隐藏"就是关掉它那一格，而且它**不可放置**（`{ placeable: false }`）——它的面板由本层画在 overlay 上，占位那一格的 body 是空盒子，所以谁把它放进自己的格子都只会得到一格空白（T25）；**左栏也可关**，而且理由与右栏同源：它由**本层**立起来（下面第 3 条），关掉之后下一次对账会把它立回来。它曾经声明 `closable: false`，那个声明是错的——策略是**类型**的属性，于是任何显示导航面板的格子都关不掉，包括用户从选择列表里给自己开的那一格（T24）；
...
- **左栏的让位要还**：核心的 `resizePane` 按"可让位的兄弟"比例分摊，而左栏声明了固定宽度，所以右栏占位时**不再**动它（T26 之后）。这条还账逻辑留着，管的是另外两种情况：柱在工作期间被**用户**改过宽度（折叠成轨），以及一个没有声明固定宽度的导航列。`rightbar.test.ts` 里那条棘轮用例仍然钉着"每开合一次侧栏窄一点"这个失败。
